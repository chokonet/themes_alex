...
// FRONT END SCRIPTS AND STYLES //////////////////////////////////////////////////////
	 ...
	 * localize scripts
	 */
	function localize_bemygirl_scripts(){
...
// ADD THUMBNAIL SUPPORT /////////////////////////////////////////////////////////////
	...
	add_image_size( 'imagen_girl', 150, 150, true );
...
// MAQUILADORES HELPER FUNCTIONS /////////////////////////////////////////////////////
	...
	/**
	 * Trae los services de la escort
	 * @param  [type] $escort_ID [id de escort]
	 * @return [type]            [nombre de servicios]
	 */
	function get_user_services($escort_ID, $type) {
		global $wpdb;

		if ($type === "services"):
			$type_query = "({$wpdb->prefix}services.type = 'service_ordinary' OR {$wpdb->prefix}services.type = 'service_extra')";
		else:
			$type_query = "{$wpdb->prefix}services.type = '$type'";
		endif;

	    return $wpdb->get_results(
			"SELECT {$wpdb->prefix}services.name FROM {$wpdb->prefix}services
					WHERE {$wpdb->prefix}services.escort_id = $escort_ID AND {$wpdb->prefix}services.value = '1'
					AND $type_query ;"
		);
	...
	function saveAttachmentData($data, $image_name){
		...
		return $upload ? $upload_dir['path'].'/'."$image_name" : false;
	}
