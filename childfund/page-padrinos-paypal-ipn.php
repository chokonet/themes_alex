<?php
	...
	$admin_email = 'noricumbo@gmail.com';
	$paypal_email = 'childfundmexico@childfundmexico.org.mx';
	//$paypal_email = 'scrub_1349979018_biz@gmail.com';
	//$sandbox = true;
	...
	$query = "SELECT txn_id FROM wp_cart WHERE txn_id = '$txn_id';";
	...
	for($i=0; $i<count($pieces); $i+=4){
		$cart_id = $pieces[$i];
		$beneficiario = $pieces[$i+1];
		$nino_id = $pieces[$i+2];
		$padrino_id = $pieces[$i+3];
		$update = "UPDATE wp_cart SET beneficiario = '$beneficiario', cart_status = '$payment_status', nino_id_chf = '$nino_id', padrino_id_chf = '$padrino_id', txn_id = '$txn_id' WHERE cart_id = '$cart_id';";
