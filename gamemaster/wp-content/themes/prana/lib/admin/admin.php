...
class PranaAdmin {
		...
		function settings_sections() {
			...
			add_settings_field( 'prana_field_menu_position', __( 'Menu Position', 'prana' ), array( &$this, 'prana_field_menu_position_fn' ), 'prana_section_blog_page', 'prana_section_blog' );
			add_settings_field( 'prana_field_menu_width', __( 'Menu Width', 'prana' ), array( &$this, 'prana_field_menu_width_fn' ), 'prana_section_blog_page', 'prana_section_blog' );
		...
		function get_settings_default()  {
			...
			$default = array(
				...
				'prana_menu_position' => 'default',
				'prana_menu_width' => 940,
		...
		/* Menu Position Range */		
		function prana_menu_position_pd() {			
			return array( 'default' => __( 'Default (Left)', 'prana' ), 'center' => __( 'Center', 'prana' ) );			
		}
		...
		function prana_options_validate( $input ) {
			...
			$prana_menu_position_pd = $this->prana_menu_position_pd();
			...
			/* Validation: prana_menu_position */
			if ( ! array_key_exists( $input['prana_menu_position'], $prana_menu_position_pd ) ) {
				 $input['prana_menu_position'] = $default['prana_menu_position'];
			}
			
			/* Validation: prana_menu_width */
			$input['prana_menu_width'] = absint( $input['prana_menu_width'] );
			if ( ! ( $input['prana_menu_width'] < 940 && $input['prana_menu_width'] > 1 ) ) {
				 $input['prana_menu_width'] = $default['prana_menu_width'];
			}
			...
			add_settings_error( 'prana_options', 'prana_options', __( 'Settings Saved.', 'prana' ), 'updated' );
		...
		function prana_field_menu_position_fn() {
			
			$prana_options = get_option( 'prana_options' );
			$items = $this->prana_menu_position_pd();
			
			echo '<select id="prana_menu_position" name="prana_options[prana_menu_position]">';
			foreach( $items as $key => $val ) {
			?>
            <option value="<?php echo $key; ?>" <?php selected( $key, $prana_options['prana_menu_position'] ); ?>><?php echo $val; ?></option>
            <?php
			}
			echo '</select>';
			echo '<div><small>'. __( '<strong>Center:</strong> You must specify the <em>Menu Width</em> according to your menu items in order to center it perfectly.', 'prana' ) .'</small></div>';
		
		}
		
		/* Menu Width Callback */
		function prana_field_menu_width_fn() {
			
			$prana_options = get_option('prana_options');
			echo '<input type="text" id="prana_menu_width" name="prana_options[prana_menu_width]" value="'. sanitize_text_field ( $prana_options['prana_menu_width'] ) .'" />';
			echo '<div><small>'. __( 'Enter the menu width to center it perfectly.', 'prana' ) .'</small></div>';
			echo '<div><small>'. __( '<strong>Valid Range:</strong> 1 - 940', 'prana' ) .'</small></div>';
